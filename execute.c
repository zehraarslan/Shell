...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/12 00:44:40 by tyildiri          #+#    #+#             */
/*   Updated: 2022/11/12 00:45:04 by tyildiri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	new_process(t_shell *shell, t_node *node)
{
	...
	if (!pid)
	{
		set_input_redirections(node, number_of_inputs_node(node));
		...
		if (node->cmd_path != NULL)
			execve(node->cmd_path, node->exec_args, g_shell.env);
		exit(1);
	}
}
